...
""" 
    Enter Inputs Here 
"""
file_name = r"C:\Users\Xiaochun.wang\Desktop\Data\Stop Loss Data.xlsx"
...
df_weekly = data_weekly[['Dates', 'SPX Index', '3M TBILL']].copy()
df_weekly.set_index("Dates", inplace=True)

""" Weekly Returns """
# Compute weekly returns of S&P 500 and 3M tbill
df_weekly['ret'] = df_weekly['SPX Index'].pct_change(1)
...
# Compute the exponential weighted moving average of daily returns
df_weekly['ret ewma'] = df_weekly['ret 1'].ewm(span=weekly_ewma_span, ignore_na=True).mean()
...
# Compute the exponential weighted moving average of daily log returns
df_weekly['log ret ewma'] = df_weekly['log ret 1'].ewm(span=weekly_ewma_span, ignore_na=True).mean()
...
def construct_stop_loss_strategy(data, x, threshold):  # x: lead/lag
    """ Construct the buy/sell strategy """
    data.loc[data['prob 0'].shift(x) >= threshold, 'pos'] = 0
    data.loc[data['prob 0'].shift(x) < threshold, 'pos'] = 1

    data['pos'][0] = 0
    data.loc[data['pos'] == data['pos'].shift(1), 'rebalance'] = 0
    data.loc[data['pos'] != data['pos'].shift(1), 'rebalance'] = 1
    data['pos'] = data['pos'].fillna(0)

    data.loc[data['pos'] == 1, 'adj ret'] = data['ret']
    data.loc[data['pos'] == 0, 'adj ret'] = data['3m tbill ret']
    data.loc[data['pos'] == 1, 'adj log ret'] = data['log ret']
    data.loc[data['pos'] == 0, 'adj log ret'] = data['3m tbill log ret']

    data['cum adj log ret'] = data['adj log ret'].cumsum()
    data['adj dollar growth'] = np.cumprod(1 + data['adj ret'])
    data['adj high value'] = data['adj dollar growth'].cummax()
    data['adj dd'] = (data['adj high value'] - data['adj dollar growth']) / data['adj high value']

    return data

def compute_stop_loss_results(data):
    stat = [data['adj ret'].mean() * 52,  # annualized returns
            (data['adj ret'].std() ** 2) * 52,  # annualized volatilities
            data['adj ret'].mean() / df_hmm['adj ret'].std() * np.sqrt(52),  # sharpe ratio
            data['adj dd'].max(),  # maximum drawdown
            data['cum adj log ret'][-1],  # cumulative log return
            data['rebalance'].sum()]
    stat = pd.DataFrame(stat)
    return stat

df = df_weekly[['ret', 'log ret', '3m tbill ret', '3m tbill log ret']].join(pto)   # without kim filter
# df = df_weekly[['ret', 'log ret', '3m tbill ret', '3m tbill log ret']].join(ptos)   # with kim filter

# Compute the statistics of benchmark index
df['cum log ret'] = df['log ret'].cumsum()
df['dollar growth'] = np.cumprod(1 + df['ret'])
df['high value'] = df['dollar growth'].cummax()
df['dd'] = (df['high value'] - df['dollar growth']) / df['high value']
benchmark_stat = [df['ret'].mean() * 52, (df['ret'].std() ** 2) * 52,
                  df['ret'].mean() / df['ret'].std() * np.sqrt(52),
                  df['dd'].max(), df['cum log ret'][-1], 1]
benchmark_stat = pd.DataFrame(benchmark_stat)

# Perform lead/lag analysis on the strategy
lead_lag_cum_log_ret = lead_lag_summary_table = pd.DataFrame()
for i in range(-4, 7, 1):
    df_hmm = construct_stop_loss_strategy(df, i, 0.4)
    cum_log_ret = df_hmm['cum adj log ret']
    lead_lag_cum_log_ret = pd.concat([lead_lag_cum_log_ret, cum_log_ret], axis=1)
    stat = compute_stop_loss_results(df_hmm)
    lead_lag_summary_table = pd.concat([lead_lag_summary_table, stat], axis=1)

benchmark_stat.index = lead_lag_summary_table.index \
    = ['Annualized Return', 'Annualized Volatility', 'Sharpe Ratio', 'Maximum Drawdown', 'Cumulative Log Return', '# of Rebalance']
lead_lag_cum_log_ret.columns = lead_lag_summary_table.columns \
...
# Plot the bar chart of Sharpe ratio lead/lag
plt.subplot(grid[0, 0])
plt.title('Sharpe Ratio Lead Lag Analysis')
plt.bar(lead_lag_summary_table.columns, lead_lag_summary_table.loc['Sharpe Ratio'])
plt.axhline(y=benchmark_stat[0]['Sharpe Ratio'], linestyle='dashed', c='indianred')
#plt.xticks(rotation=45)
plt.grid()

# Plot the bar chart of maximum drawdown lead/lag
plt.subplot(grid[0, 1])
# plt.rcParams["figure.figsize"] = [6, 4]
plt.title('Maximum Drawdown Lead Lag Analysis')
plt.bar(lead_lag_summary_table.columns, lead_lag_summary_table.loc['Maximum Drawdown'])
plt.axhline(y=benchmark_stat[0]['Maximum Drawdown'], linestyle='dashed', c='indianred')
#plt.xticks(rotation=45)
plt.grid()

# Plot the time series of cumulative log returns
plt.subplot(grid[1, :])
#plt.rcParams["figure.figsize"] = [12, 4]
plt.title('Cumulative Log Return Lead Lag Analysis')
plt.plot(lead_lag_cum_log_ret['3'], label='Lag 3')
plt.plot(lead_lag_cum_log_ret['2'], label='Lag 2')
plt.plot(lead_lag_cum_log_ret['1'], label='Lag 1')
plt.plot(lead_lag_cum_log_ret['0'], label='No Lag/Lead')
plt.plot(df['cum log ret'], label='S&P 500')
plt.xlabel('Dates')
...
df_hmm = construct_stop_loss_strategy(df, 1, 0.4)
stat = compute_stop_loss_results(df_hmm)

df_hmm.loc[df_hmm['prob 0'].shift(1) >= 0.5, 'pos'] = 0
df_hmm.loc[df_hmm['prob 0'].shift(1) < 0.5, 'pos'] = 1
df_hmm['pos'][0] = 0
df_hmm.loc[df_hmm['pos'] == df_hmm['pos'].shift(1), 'rebalance'] = 0
df_hmm.loc[df_hmm['pos'] != df_hmm['pos'].shift(1), 'rebalance'] = 1
df_hmm.loc[df_hmm['pos'] == 1, 'adj ret'] = df_hmm['ret']
df_hmm.loc[df_hmm['pos'] == 0, 'adj ret'] = df_hmm['3m tbill ret']
df_hmm.loc[df_hmm['pos'] == 1, 'adj log ret'] = df_hmm['log ret']
df_hmm.loc[df_hmm['pos'] == 0, 'adj log ret'] = df_hmm['3m tbill log ret']
df_hmm['dollar growth'] = np.cumprod(1 + df_hmm['adj ret'])
df_hmm['high value'] = df_hmm['dollar growth'].cummax()
df_hmm['dd'] = (df_hmm['high value'] - df_hmm['dollar growth']) / df_hmm['high value']
df_hmm['cum log ret'] = df_hmm['log ret'].cumsum()
df_hmm['cum adj log ret'] = df_hmm['adj log ret'].cumsum()

# Generate the summary table
stat1 = [df_hmm['adj ret'].mean() * 52,  # annualized returns
         (df_hmm['adj ret'].std() ** 2) * 52,  # annualized volatilities
         df_hmm['adj ret'].mean() / df_hmm['adj ret'].std() * np.sqrt(52),  # sharpe ratio
         df_hmm['dd'].max(),  # maximum drawdown
         df_hmm['cum adj log ret'][-1],  # cumulative log return
         df_hmm['rebalance'].sum()]   # num of rebalances
...
plt.hist(df_hmm['ret'].values, bins=100, histtype='bar', density=True, ec='lightsalmon', alpha=0.75, label='S&P 500')
plt.hist(df_hmm['adj ret'].values, bins=100, histtype='bar', density=True, ec='lightsteelblue', alpha=0.75, label='HMM')
plt.xlim(-0.1, 0.1)
plt.legend()

# Get the quantiles of S&P 500 returns and strategy returns
stat = pd.DataFrame([[np.quantile(df_hmm['ret'], 0), np.quantile(df_hmm['ret'], 0.25), np.quantile(df_hmm['ret'], 0.5),
                      np.quantile(df_hmm['ret'], 0.75), np.quantile(df_hmm['ret'], 1)],
                     [np.quantile(df_hmm['adj ret'], 0), np.quantile(df_hmm['adj ret'], 0.25), np.quantile(df_hmm['adj ret'], 0.5),
                      np.quantile(df_hmm['adj ret'], 0.75), np.quantile(df_hmm['adj ret'], 1)]])
